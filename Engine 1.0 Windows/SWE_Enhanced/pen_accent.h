...
//
// Pen_accent.h creates a modified sphere to create the appearance of the sphere-shaped accent of a pen. Vertices are defined, and vao and vbo objects
// are established with the function PenAccent(). The draw() function is called to render the pen accent. The class has constructors and deconstructors
        ...
        { // generate VBO, VAO, EBO
            ...
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
                {
                    ...
                    // Here, I have multiplied by .0083 since I want the object to be very very small.
                    float xPos = std::cos(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .0083f + .38f;
                    float yPos = std::cos(ySegment * PI) * .0083f + 1.23f;
                    float zPos = std::sin(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .0083f + 1.17f;
