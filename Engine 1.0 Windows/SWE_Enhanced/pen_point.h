...
//
// Pen_point.h creates a modified sphere to create the appearance of the point of a pen. Vertices are defined, and vao and vbo objects
// are established with the function PenPoint(). The draw() function is called to render the pen. The class has constructors and deconstructors
        ...
        if (VAO == 0)
        {
            ...
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
                {
                    ...
                    float xPos = std::cos(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .033f + -.478f;
                    float yPos = std::cos(ySegment * PI) * .0033f + 1.23f;
                    float zPos = std::sin(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .0033f + 1.15f;
